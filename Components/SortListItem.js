        ...
        const panResponder = PanResponder.create(
            {
                ...
                onPanResponderGrant: () => {
                    ...
                    let offset = props.horizontal === true ? {x: position.__getValue().x, y: 0} : {
                        x: 0,
                ...
                onPanResponderTerminate: () => {// When user's touch/gesture is moved outside of direct parent container
                    this.props.setCancelContentTouches(true);
                }
            });
        ...
        Animated.timing(this.state.position, {toValue: newPos, duration: 300}).start(() => {
            ...
            if (highlight === true) {
                Animated.timing(this.state.position, {
                    toValue: (this.props.horizontal === true ? {x: 0, y: 25} : {
                        x: 25,
                        y: 0
                    }), duration: 150,
                }).start(() => {
        ...
        return (
            <Animated.View
                ...
                onLayout={(event) => this.setState(
                    {
                        ...this.state,
                        itemSize: (this.props.horizontal === true ? event.nativeEvent.layout.width : event.nativeEvent.layout.height)
                    })}
            >
