...
export default function SortList(props) {
  ...
  useEffect(() => {
    if (items === null) {
      if (alreadyHighlighted === false) setAlreadyHighlighted(true);
      else if (alreadyHighlighted === true && lastShiftingItemIndex !== -1)
        setLastShiftingItemIndex(-1);
      setItems(initItems(props.data));
      if (
        typeof scrollList.current !== "undefined" &&
        scrollList.current !== null
      )
  ...
  const onSave = (id, itemSize) => {
    if (isMoving === true) setIsMoving(false);
    const itemsWithoutCorrectIndexes = items.sort((x, y) => x.index - y.index);
    const shiftingItem = itemsWithoutCorrectIndexes.filter(
    ...
    const posToScrollTo = Math.max(
      0,
      newItems.filter((x) => parseInt(x.id) === parseInt(id))[0].index *
        itemSize -
        containerSize / 2
    );
  ...
  const onMove = (id, draggablePosition, itemSize, isEdging, scrollPos) => {
    if (isMoving === false) setIsMoving(true);
    const newItems = [...items];
    let shiftingItem = newItems.filter(
      (x) => parseInt(x.id) === parseInt(id)
    )[0];
    // Calculate shiftingItem new index
    let shiftingItemIndex = Math.trunc(
      (draggablePosition + itemSize / 2) / itemSize
    );
    ...
    if (isEdging !== 0) {
      const minIndex = Math.min(
        ...newItems.filter((x) => x.id !== id).map((x) => x.index)
      );
      ...
      const maxShiftPrev =
        newItems.length -
        (Math.trunc(
          (newItems.length * itemSize - (scrollPos + containerSize)) / itemSize
        ) +
          1);
    ...
    if (shiftingItemIndex !== shiftingItem.index) {
      let toSnap = [];
      if (shiftingItemIndex < shiftingItem.index) {
        toSnap = newItems.filter(
          (x) =>
            x.id !== id &&
            x.index < shiftingItem.index &&
            shiftingItemIndex <= x.index
        );
        toSnap.forEach((x) => {
          x.index = x.index + 1;
        });
  ...
  return (
    <View
      style={[
        props.horizontal === true ? { height: "100%" } : { width: "100%" },
        props.style,
      ]}
      onLayout={(event) =>
        setContainerSize(
        ...
        style={[
          ...
          props.horizontal === true
            ? {
                ...
                width:
                  isMoving === true
                    ? (containerSize * props.edgeZonePercent) / 100
                    : 0,
              }
            : {
                top: 0,
